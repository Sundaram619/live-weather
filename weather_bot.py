"""
Live Weather Prediction Bot
============================
Fetches real-time weather for any city using Open-Meteo API.
No API key required.
...
def build_session(retries=3, backoff=0.5):
    ...
    backoff_factor adds exponential delay between retries.
    """
    s = requests.Session()
    retry = Retry(
        total=retries,
...
def geocode_city(session, city: str, timeout=5):
    ...
    Returns dict with: name, country, lat, lon, timezone
...
def get_weather(session, lat, lon, timezone="auto", timeout=5):
    """
    Fetch current weather + today's daily summary from Open-Meteo Forecast API.

    Current fields: temperature, humidity, feels-like, weather code, wind speed
    Daily fields:   max temp, min temp, precipitation sum
    """
...
def print_weather(city_info, weather_json):
    """Format and display the weather data in a readable format."""
